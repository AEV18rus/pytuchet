...
## Текущая архитектура
- Идентификация основана на `telegram_id` (заголовок `x-telegram-id`).
- Fallback-пользователь для браузера без Telegram (`telegram_id = 87654321`).
- Нет полей для пароля/сессий в БД.
- Хранение пользователя в `localStorage` на клиенте; проверка наличия в БД через `/api/user/profile`.
