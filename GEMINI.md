...
5.  **Использование навыков (Skills):** Если для текущей задачи (создание фич, рефакторинг, отладка и т.д.) существует подходящий навык в папке `.agent/skills`, я **ОБЯЗАН** прочитать его `SKILL.md` и следовать его инструкциям. Это обеспечивает соответствие лучшим практикам и архитектурным стандартам.
...
Если задача имеет файл плана выполнения (например, `.md`) или чек-лист:
1. Сразу после выполнения каждого шага я ОБЯЗАН отметить его выполненным (заменить `[ ]` на `[x]`).
2. Это должно происходить автоматически по завершении этапа, без дополнительного напоминания.
